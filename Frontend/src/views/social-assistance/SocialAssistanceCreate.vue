...
const handleSubmit = async () => {
    await createSocialAssistance(socialAssistace.value)
    ...
    <form @submit.prevent="handleSubmit" id="myForm" class="capitalize">
        <div class="shrink-0 rounded-3xl p-6 bg-white flex flex-col gap-6 h-fit">
            ...
            <section id="Buttons" class="flex items-center justify-end gap-4">
                ...
                <Button type="submit" label="Create Now" :loading="loading" class="py-[18px] rounded-2xl disabled:bg-desa-grey w-[180px] text-center flex justify-center font-medium text-white bg-desa-dark-green transition-all duration-300">Create Now</Button>
            </section>
        </div>
    </form>
</template>