    ...
    <div id="Modal-Delete" class="modal fixed inset-0 h-screen z-40 flex bg-[#080C1ACC]" v-if="show">
        <div id="Alert" class="flex flex-col w-[335px] shrink-0 rounded-2xl overflow-hidden bg-white m-auto">
            <div class="flex items-center justify-between p-4 gap-3 bg-desa-black">
                <p class="font-medium leading-5 text-white">Hapus {{ title }}?</p>
            ...
            <div class="flex flex-col p-4 gap-3">
                ...
                <div class="flex items-center gap-3">
                    <button @click="$emit('close')" class="btn-close-modal flex items-center h-14 rounded-2xl py-3 px-8 gap-[10px] border border-desa-background hover:bg-desa-black hover:text-white transition-setup">
                        <span class="font-semibold text-sm">Batal</span>
                    </button>
                    <button class="flex items-center h-14 rounded-2xl py-3 px-8 gap-[10px] bg-desa-red w-full" @click="$emit('confirm')" :disabled="loading">
