...
<template>
    <div class="card flex flex-col gap-4 rounded-3xl p-6 bg-white">
        <div class="flex items-center justify-between">
            ...
            <div class="badge rounded-full p-3 gap-2 flex w-[100px] justify-center shrink-0 bg-desa-yellow" v-if="item.status === 'pending'">
                <span class="font-semibold text-xs text-white uppercase">Menunggu</span>
            </div>
            <div class="badge rounded-full p-3 gap-2 flex w-[100px] justify-center shrink-0 bg-desa-green" v-if="item.status === 'approved'">
                <span class="font-semibold text-xs text-white uppercase">Disetujui</span>
            </div>
            <div class="badge rounded-full p-3 gap-2 flex w-[100px] justify-center shrink-0 bg-desa-red" v-if="item.status === 'rejected'">
        ...
        <div class="flex items-center gap-6 justify-between">
            ...
            <div class="flex items-center gap-3 justify-end w-[252px] shrink-0">
                <RouterLink :to="{name: 'social-assistance'}" class="flex items-center shrink-0 gap-[10px] rounded-2xl py-4 px-6 bg-desa-black">
